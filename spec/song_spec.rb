...
describe '#Song' do
  ...
  before(:each) do
    Album.clear()
    Song.clear()
    @album = Album.new("Giant Steps", nil, "Johm Coltrane", "Jazz", "1959")
    @album.save()
  end

  describe('#album') do
  it("finds the album a song belongs to") do
    song = Song.new("Naima", @album.id, nil, "Me")
    song.save()
  ...
  describe('.find_by_album') do
    it("finds songs for an album") do
      album2 = Album.new("Blue", nil, "Johm Coltrane", "Jazz", "1959")
      album2.save
      song = Song.new("Naima", @album.id, nil, "Me")
      song.save()
      song2 = Song.new("California", album2.id , nil, "Me")
      song2.save()
  ...
  describe('#==') do
    it("is the same song if it has the same attributes as another song") do
      song = Song.new("Naima", @album.id, nil, "Me")
      song2 = Song.new("Naima", @album.id, nil, "Me")
  ...
  describe('.all') do
    it("returns a list of all songs") do
      song = Song.new("Giant Steps", @album.id, nil, "Me")
      song.save()
      song2 = Song.new("Naima", @album.id, nil, "Me")
      song2.save()
